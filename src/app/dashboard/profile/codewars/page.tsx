...
import { DiamondsCollectButton } from '../../leaderboard/components/DiamondsCollectButton';

export default async function CodewarsProfilePage() {
  let { data: profileData } = await getCodewarsProfileData();
  return (
    <div className='grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4'>
      {profileData?.isConnected ? <StatCards /> : <NotConnectedGrid />}
      <DiamondsCollectButton count={100} />
    </div>
  );
}
