...
import { productsTableSchema } from '../../schemas';
import { z } from 'zod';
import React from 'react';

interface Props {
  table: Table<z.infer<typeof productsTableSchema>>;
  data: z.infer<typeof productsTableSchema>[];
  setData: React.Dispatch<
    React.SetStateAction<z.infer<typeof productsTableSchema>[]>
  >;
}

const ProductsTabContent = ({ table, data, setData }: Props) => {
  const productsSortableId = React.useId();
  ...
  function productsHandleDragEnd(event: DragEndEvent) {
    const { active, over } = event;
    if (active && over && active.id !== over.id) {
      setData((data) => {
        const oldIndex = productsDataIds.indexOf(active.id);
        const newIndex = productsDataIds.indexOf(over.id);
    ...
    >
      ...
      <div className='max-h-[70vh] overflow-auto rounded-lg border'>
        <DndContext
          ...
          onDragEnd={productsHandleDragEnd}
          sensors={productsSensors}
          id={productsSortableId}
        >
          <ShadCNTable>
